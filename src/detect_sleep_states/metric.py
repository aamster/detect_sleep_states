...
def score(
    ...
    if not is_numeric_dtype(submission[time_column_name]):
        raise ParticipantVisibleError(
            f"Submission column '{time_column_name}' must be of numeric type."
        )
    if not is_numeric_dtype(submission[score_column_name]):
        raise ParticipantVisibleError(
...
def match_detections(
    ...
    return detections_sorted, list(gts_matched)
...
def event_detection_ap(
    ...
    aggregation_keys = pd.DataFrame(
        [(event, tolerance, series_id)
         for event in tolerances.keys()
         for tolerance in tolerances[event]
         for series_id in ground_truths[series_id_column_name].unique()],
    ...
    detections_matched = []
    ground_truths_matched = []
    for key in aggregation_keys.itertuples(index=False):
        dets = detections_grouped.get_group(key)
        gts = ground_truths_grouped.get_group(key)

        dets, gts_matched = match_detections(
            tolerance=dets['tolerance'].iloc[0],
            ground_truths=gts,
            detections=dets)
        detections_matched.append(dets)
        ground_truths_matched.append(gts_matched)
    detections_matched = pd.concat(detections_matched)

    ground_truths_matched = [gt[i] for gt in ground_truths_matched for i in range(len(gt))]
    ground_truths_matched = pd.DataFrame(ground_truths_matched)

    # Compute AP per event x tolerance group
    event_classes = ground_truths[event_column_name].unique()
    ap_table = (
        detections_matched[detections_matched['event'].isin(event_classes)]
    ...
    return mean_ap, detections_matched, ground_truths_matched
